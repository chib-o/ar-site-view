(function(){
	...
	function riskmachSyncFactory($q, $http, $filter, $timeout, riskmachDatabasesFactory, authFactory, rmUtilsFactory)
	{
		...
		factory.sync_collection = {
			...
			staging: {
				...
				addStagedRecord: function(record, record_type){
					...
					if( factory.sync_collection.staging.staged_records.hasOwnProperty(record._id) ) 
					{
						console.log("RECORD ALREADY STAGED");
						return;
					}

					if( factory.sync_collection.staging.staged_files.hasOwnProperty(record._id) ) 
					{
						console.log("THIS FILE HAS ALREADY BEEN STAGED");
						return;
					}
		...
		factory.fetchProjectSyncData = function(project_id)
		{
			...
			factory.fetch.projectRecord(project_id).then(function(project_record){
				...
				factory.fetchCollection.projectAssets(project_id).then(function(assets){
					factory.utils.formatAssetsChildImportError(assets);
					...
					factory.fetchCollection.qcCheckRecords(project_record).then(function(qc_check_records) {
						...
						// factory.fetchCloseoutData(project_record).then(function() {
		...
		factory.fetchCloseoutData = function(project_record) 
		{
			var defer = $q.defer();
			var fetch_defer = $q.defer();

			var stages = ['closeout_risks','closeout_evidence'];
			...
			function fetchNextPage(defer, active_index) {
				...
				if( stages[active_index] == 'closeout_risks' ) {

					factory.fetchCollection.closeoutRiskRecords(project_record).then(function(closeout_risks) {

						active_index++;

						fetchNextPage(defer, active_index);
				...
				if( stages[active_index] == 'closeout_evidence' ) {

					factory.fetchCollection.closeoutEvidence().then(function(){
		...
		factory.fetchCollection = {
			...
			closeoutRiskRecords: function(project_record) {
				var defer = $q.defer();

				if( !project_record.hasOwnProperty('actioned_ra_meta') || !project_record.actioned_ra_meta || !project_record.actioned_ra_meta.length ) {
					defer.resolve();
					return defer.promise;
				}
				...
				fetchNextCloseoutRisk(fetch_defer, 0).then(function() {

					factory.sync_collection.staging.addStagedRecordBulk(closeout_risks, 'closeout_risk');
				...
				function fetchNextCloseoutRisk(defer, active_index) {

					if( active_index > project_record.actioned_ra_meta.length - 1 ) {
						defer.resolve();
						return defer.promise;
					}
					...
					db.get(project_record.actioned_ra_meta[active_index].app_id).then(function(risk_doc) {

						// IF NOT IMPORTED
						if( !risk_doc.date_content_imported ) {
							closeout_risks.push(risk_doc);
						}

						active_index++;

						fetchNextCloseoutRisk(defer, active_index);
			...
			closeoutEvidence: function() {
				...
				var closeout_evidence = [];

				fetchNextPage(fetch_defer).then(function() {

					factory.sync_collection.staging.addStagedRecordBulk(closeout_evidence, 'closeout_evidence');
					defer.resolve(closeout_evidence);
					...
					db.allDocs(options).then(function(result) {
						...
						if( result && result.rows.length > 0 ) {
							...
							while(i < len) {
								...
								if( result.rows[i].doc.record_type != 'ra_closeout_evidence' ) {
									errors++;
								}
								...
								// IF RISK CLOSEOUT NOT IN SYNC
								if( !factory.sync_collection.staging.staged_ids.hasOwnProperty('closeout_risk') || factory.sync_collection.staging.staged_ids['closeout_risk'].indexOf(result.rows[i].doc.record_id) === -1 ) {
									errors++;
								}

								if( errors == 0 ) {
									filtered_evidence.push(result.rows[i].doc);
								}

								i++;
							}

							closeout_evidence.push(...filtered_evidence);

							options.startkey = result.rows[ result.rows.length - 1 ].id;
							options.skip = 1;

							result.rows = null;
							filtered_evidence = null;
		...
		factory.import_requests = {
			...
			importRecord: function(staging_record) {
				...
				// switch(staging_record.record_type) {
				// 	case 'qc_check_record': 
				// 		// RUN BESPOKE IMPORT ROUTINE
				// 		factory.import_requests.importQualityCheckRecord(staging_record).then(function() {
				// 			defer.resolve();
				// 		}, function(error) {
				...
				// 	case 'closeout_risk': 
				// 		// RUN BESPOKE IMPORT ROUTINE
				// 		factory.import_requests.importCloseoutRiskRecord(staging_record).then(function() {
				// 			defer.resolve();
				// 		}, function(error) {
				// 			defer.reject(error);
				// 		});
				// 		break;
				// 	case 'closeout_evidence':
				// 		// RUN BESPOKE IMPORT ROUTINE
				// 		break;
				// 	default: 
				// 		factory.import_requests.doImportRecord(staging_record).then(function() {
			...
			doImportRecord: function(staging_record) {
				...
				function importRecord(staging_record, defer){
					...
					{
						if( staging_record.record_type == 'media' ) {
							// FIND ASSET AND MARK IMPORT ERRORED
							var asset_record = factory.utils.findMediaRecordsStagedAssetRecord(staging_record.record);

							console.log("FOUND MEDIA RECORD'S ASSET");
							console.log(asset_record);

							factory.dbUtils.snapshot_asset.saveAssetChildRecordImportError(asset_record, staging_record.record, staging_record.record_type, staging_record.attempt_meta.last_error).then(function() {
								defer.resolve();
							}, function(error) {
								defer.resolve();
							});
	    ...
	    //         	}
			...
			importCloseoutRiskRecord: function(staging_record) {
				var defer = $q.defer();
				var import_defer = $q.defer();

				var endpoint = factory.test_request_prefix;
				var params = null;
				var intended_closeout_action = null;

				if( staging_record.record.hasOwnProperty('intended_closeout_action') && staging_record.record.intended_closeout_action ) {
					intended_closeout_action = staging_record.record.intended_closeout_action;
				}

				if( intended_closeout_action == 'submit_for_review' ) {
					
					endpoint += 'laravel/public/webapp/v1/CloseoutAssessment';

					if( !staging_record.record.hasOwnProperty('submit_for_review_info') || !staging_record.record.submit_for_review_info ) {
						defer.reject("Risk assessement has no closeout information");
						return defer.promise;
					}  

					params = {
						assessment_id: staging_record.record.rm_id, 
						date_closed: staging_record.record.submit_for_review_info.date_closed, 
						no_evidence_reason: staging_record.record.submit_for_review_info.no_evidence_reason
					};

				}

				if( intended_closeout_action == 'revise_assessment' ) {

					endpoint += 'laravel/public/webapp/v1/ReviseCloseoutAssessment';

					if( !staging_record.record.hasOwnProperty('review_info') || !staging_record.record.review_info ) {
						defer.reject("Risk assessment has no revision information");
						return defer.promise;
					}

					params = {
						assessment_id: staging_record.record.rm_id, 
						revision_info: staging_record.record.revision_info
					};

				}

				if( intended_closeout_action == 'review_assessment' ) {

					endpoint += 'laravel/public/webapp/v1/ApproveAssessmentCloseout';

					if( !staging_record.record.hasOwnProperty('review_info') || !staging_record.record.review_info ) {
						defer.reject("Risk assessment has no review information");
						return defer.promise;
					}

					params = {
						assessment_id: staging_record.record.review_info.assessment_id, 
						approved: staging_record.record.review_info.approved,
						description: staging_record.record.review_info.description
					};

				}

				importCloseoutRecord(staging_record, import_defer).then(function(sync_result) {
					staging_record.record.date_record_imported = new Date().getTime();
					staging_record.record.imported = true;
					//INCREMENT SYNCED
					factory.sync_collection.incrementStat('total_imported', 1);
				...
				function importCloseoutRecord(staging_record, defer) {
					...
					if( staging_record.attempt_meta.num_attempts > staging_record.attempt_meta.attempt_limit ) {
						defer.reject("Error importing closeout risk " + staging_record.record.rm_id + " [attempt limit exceed]: " + staging_record.attempt_meta.last_error);
						return defer.promise;
					}
					...
					.success(function(data, status, headers, config) {
						...
						if( data.error == true )
						{
							...
							staging_record.attempt_meta.num_attempts++;
							staging_record.attempt_meta.last_error = data.error_messages[0];
		            		importCloseoutRecord(staging_record, defer);
						}
						else
						{
							factory.import_requests.saveImportResult(staging_record, data).then(function(up_result){
			...
			saveImportResult: function(staging_record, result){
				...
				if( staging_record.record_type == 'closeout_risk' ) 
				{
					factory.dbUtils.assessments.saveCloseoutRiskImportResult(staging_record.record, result).then(function() {
						defer.resolve();
					}, function(error) {
						defer.reject(error);
					});
		...
		factory.utils = {
			...
			findMediaRecordsStagedAssetRecord: function(media_record) {
				var asset_record = null;
				var parent_record = null;

				if( media_record.record_type == 'asset' ) {
					asset_record = factory.utils.findStagedImportRecord(media_record.record_id, 'asset');
				}

				// RISK RECORD
				if( media_record.record_type == 'assessment' ) {
					parent_record = factory.utils.findStagedImportRecord(media_record.record_id, 'assessment');

					if( parent_record ) {
						asset_record = factory.utils.findStagedImportRecord(parent_record.asset_id, 'asset');
					}
				}

				// TASK RECORD
				if( media_record.record_type == 'task' ) {
					parent_record = factory.utils.findStagedImportRecord(media_record.record_id, 'task');
				...
				if( media_record.record_type == 'assessment_hazard' ) {
					parent_record = factory.utils.findStagedImportRecord(media_record.record_id, 'mr_hazard');
				...
				if( media_record.record_type == 'control_item' ) {
					parent_record = factory.utils.findStagedImportRecord(media_record.record_id, 'mr_control');
			...
			findErroredImportStagedAssets: function() {
				var assets = [];

				Object.keys(factory.sync_collection.staging.import_staged).forEach(function(current_key){

					// IF ASSET RECORD
					if( factory.sync_collection.staging.import_staged[current_key].record_type == 'asset' ) {

						// IF CHILD RECORD IMPORT ERROR
						if( factory.sync_collection.staging.import_staged[current_key].record.hasOwnProperty('child_import_error') && factory.sync_collection.staging.import_staged[current_key].record.child_import_error == 'Yes' ) {
							assets.push( factory.sync_collection.staging.import_staged[current_key].record );
						}
			...
			formatAssetsChildImportError: function(assets) {
				var i = 0;
				var len = assets.length;

				while(i < len) {
					assets[i].child_import_error = null;
					assets[i].child_import_error_data = null;
			...
			allStagedImportRecordsSynced: function() {
				var records_synced = true;

				Object.keys(factory.sync_collection.staging.import_staged).forEach(function(current_key) {

					if( !factory.sync_collection.staging.import_staged[current_key].record.hasOwnProperty('mid_record_id') || !factory.sync_collection.staging.import_staged[current_key].record.mid_record_id ) {
		...
		factory.dbUtils = {
			...
			markStagedRecordContentsImported: function(staging_record) {
				...
				// IF MEDIA RECORD NOT SUCCESSFULLY IMPORTED
				if( staging_record.record_type == 'media' && !staging_record.record.date_record_imported ) {
					defer.resolve(staging_record);
					return defer.promise;
				}

				// IF ASSET RECORD HAS CHILD IMPORT ERROR
				if( staging_record.record_type == 'asset' && staging_record.record.hasOwnProperty('child_import_error') && staging_record.record.child_import_error == 'Yes' ) {
					defer.resolve(staging_record);
					return defer.promise;
				}
			...
			snapshot_asset: {
				...
				saveAssetChildRecordImportError: function(asset_record, child_record, child_record_type, error) {
					var defer = $q.defer();

					var error_data = {
						child_record_id: child_record._id,
						child_mid_record_id: child_record.mid_record_id, 
						child_record_type: child_record_type,
						error_message: error,
						date: new Date().getTime()
					}

					asset_record.child_import_error = 'Yes';
					asset_record.child_import_error_data = error_data;

					riskmachDatabasesFactory.databases.collection.assets.put(asset_record).then(function(result) {

						console.log("MARKED ASSET CHILD IMPORT ERROR");
			...
			assessment: {
				...
				saveCloseoutRiskImportResult: function(record, result) {
					var defer = $q.defer();
					...
					if( record.intended_closeout_action == 'submit_for_review' ) {
						record.submit_for_review_info = null;
					}

					if( record.intended_closeout_action == 'revise_assessment' ) {
						record.revise_info = null;
					}

					if( record.intended_closeout_action == 'review_assessment' ) {
						record.review_info = null;
					}

					db.put(record).then(function(db_result) {

						record._id = db_result.id;
						record._rev = db_result.rev;
